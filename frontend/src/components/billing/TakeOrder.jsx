...
const TakeOrder = () => {
  ...
  const calculateTotals = () => {
    ...
    // Calculate GST if enabled in settings
    const gstEnabled = settings.gstEnabled || false;
    const gstPercent = settings.defaultGstPercent || 0;
    ...
    return { subtotal, discountAmount, gstAmount, gstPercent, gstEnabled, total, grandTotal, roundOff };
  };
  ...
  const generateBill = async () => {
    ...
    const { subtotal, discountAmount, gstAmount, gstPercent, grandTotal, roundOff } = calculateTotals();
  ...
  const { subtotal, discountAmount, gstAmount, gstEnabled, gstPercent, grandTotal } = calculateTotals();
  ...
  const cartColumns = [
    ...
    },
    {
      ...
      key: 'quantity',
      render: (_, record) => (
        <Space size={4} className="mobile-qty-control">
          <Button
            size="small"
            icon={<span style={{ fontSize: 16, fontWeight: 700 }}>−</span>}
            onClick={(e) => {
              e.stopPropagation();
              updateQuantity(record._id, record.quantity - 1);
            }}
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              const maxStock = record.currentStock !== undefined && record.currentStock !== null ? record.currentStock : Infinity;
              if (record.quantity < maxStock) {
                updateQuantity(record._id, record.quantity + 1);
              }
            }}
            disabled={record.currentStock !== undefined && record.currentStock !== null && record.quantity >= record.currentStock}
            style={{
              minWidth: 32,
              minHeight: 32,
              padding: 0,
    ...
    <Layout>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ marginTop: -5, marginBottom: 16, color: '#666' }}>
          Fast and easy dine-in billing interface with real-time cart
        </p>
        ...
        <Col xs={24} lg={14}>
          ...
          <Card
            ...
            extra={
              cartItems.length > 0 && (
                ...
                >
                  <span className="desktop-text">Clear All</span>
          ...
          >
            ...
            <div className="cart-mobile-view">
              {cartItems.length === 0 ? (
                <Empty
                  description="No items in cart. Start adding items!"
                  image={Empty.PRESENTED_IMAGE_SIMPLE}
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <Tag color="purple" size="small">{item.subCode}</Tag>
                        </div>
                        <Button
                          type="text"
                          danger
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ fontSize: 12, color: '#999', marginBottom: 4 }}>Price</div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <Button
                            ...
                            type="primary"
                            onClick={() => {
                              const maxStock = item.currentStock !== undefined && item.currentStock !== null ? item.currentStock : Infinity;
                              if (item.quantity < maxStock) {
                                updateQuantity(item._id, item.quantity + 1);
                              }
                            }}
                            disabled={item.currentStock !== undefined && item.currentStock !== null && item.quantity >= item.currentStock}
                            style={{
                              minWidth: 44,
                              minHeight: 44,
                              padding: 0,
            ...
            <Space direction="vertical" style={{ width: '100%' }} size="large">
              ...
              <div style={{ background: '#f5f7fa', padding: 16, borderRadius: 8 }}>
                ...
                {gstEnabled && gstAmount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, color: '#52c41a', fontWeight: 500 }}>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="print" type="primary" size="large" icon={<PrinterOutlined />} onClick={() => printThermalBill(billPreview, settings)}>
            Print Bill
          </Button>
        ]}
        width={450}
        ...
        {billPreview && (
          <div style={{ padding: '20px 0' }}>
            <div style={{ textAlign: 'center', marginBottom: 20, paddingBottom: 20, borderBottom: '2px dashed #d9d9d9' }}>
              <h2 style={{ margin: 0, fontSize: 24 }}>{settings.shopName || 'Juicy'}</h2>
              {settings.shopAddress && <div style={{ color: '#666', fontSize: 12 }}>{settings.shopAddress}</div>}
              {settings.shopMobile && <div style={{ color: '#666', fontSize: 12 }}>Contact: {settings.shopMobile}</div>}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {billPreview.gstAmount > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, color: '#52c41a' }}>
                  <span>GST ({billPreview.gstPercent || settings.defaultGstPercent}%):</span>
                  <span>+₹{billPreview.gstAmount.toFixed(2)}</span>
                </div>
              )}
      ...
      {/* Mobile-specific styles for cart and order taking */}
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          /* Cart table mobile optimizations (fallback) */
          .mobile-qty-control {
          ...
          .mobile-qty-input .ant-input-number-input {
